...
## 5. 表示フォーマット（k/m/b/t）の確認

1. 残高または価格が1000未満のプレイヤー/商品で、GUI・チャットの表示が桁区切り無しの生の数値（小数点以下は非ゼロのときだけ1桁）であることを確認する。
2. 撃破報酬の繰り返し取得やVault経由の外部コマンドなどで残高を1,000以上・1,000,000以上・1,000,000,000以上・1,000,000,000,000以上に到達させる（もしくは高額なショップ商品の価格表示で確認する）。それぞれ `k`（千）・`m`（百万）・`b`（十億）・`t`（一兆）表記に切り替わることを確認する（例: `1500` → `1.5k`、`2000000` → `2m`）。
3. ショップNPCの購入確認画面（lore）と購入成功メッセージの金額表示が一致し、どちらも `MoneyFormat` 表記であることを確認する。
4. **DBに保存される実際の残高は丸められていない**ことを確認する（表示だけがk/m/b/t表記になり、内部計算・`setBalance`/`getBalance` の値は生の `double` のまま）。
